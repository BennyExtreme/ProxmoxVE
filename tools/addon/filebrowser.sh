...
DB_PATH="/usr/local/BennyExtreme/filebrowser.db"
DEFAULT_PORT=8080
...
ExecStartPre=/bin/touch /usr/local/BennyExtreme/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/BennyExtreme/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/BennyExtreme/filebrowser.db -p ${PORT}
Restart=always
